...
*Setup instructions for a Gitian build of Trione Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Trione
...
- [Building Trione Core](#building-trione-core)
...
Clone the git repositories for Trione Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/trionecoin/trione
```
...
in the Trione Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Trione Core
----------------

To build Trione Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Trione Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/trione/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/trionecoin/trione
...
For example:
```bash
URL=https://github.com/crowning-/trione.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit trione=${COMMIT} --url trione=${URL} ../trione/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit trione=${COMMIT} --url trione=${URL} ../trione/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit trione=${COMMIT} --url trione=${URL} ../trione/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the trione git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../trione/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/trione.git
SIGPATH=/some/root/path/trione-detached-sigs.git

./bin/gbuild --url trione=${BTCPATH},signature=${SIGPATH} ../trione/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/trione-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/trione-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/trione-osx-build.assert
```
...
[trione/gitian.sigs](https://github.com/trionecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
