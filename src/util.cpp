...
const char * const BITCOIN_CONF_FILENAME = "trione.conf";
const char * const BITCOIN_PID_FILENAME = "trioned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "trione" is a composite category enabling all Trione-related debug output
            if(ptrCategory->count(string("trione"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TrioneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TrioneCore
    // Mac: ~/Library/Application Support/TrioneCore
    // Unix: ~/.trionecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TrioneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TrioneCore";
#else
    // Unix
    return pathRet / ".trionecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty trione.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override trione.conf
